...
function PolinomioTaylor() {
    ...
    const [punto, setPunto] = useState('0.5');
    const [error, setError] = useState('0.05');
    ...
    const calcularTaylor = () => {
        ...
        try {
            ...
            const _paso = [{
                ...
                valor_experimental: fn_eval + valor_x,
                polinomio: 1,
                tolerancia: "",
                errorPorcentual: 0,
                isT: false,
            ...
            while (!encontrado) {
                let sum = valor_x
                if (_paso.length >= 1) sum = sum - valor_x
                sum += _paso[_paso.length - 1].polinomio + (Math.pow(valor_x, i) / math.factorial(nf))
                Acum = sum
                EA = fn_eval - sum
            ...
            Acum = _paso[_paso.length - 1].polinomio + (Math.pow(valor_x, i) / math.factorial(i))
            EA = fn_eval - Acum;
    ...
    <Form.Control
        ...
        <td>{paso.x < 1 ? '' : `${paso.errorPorcentual.toFixed(10)}%`}</td>
